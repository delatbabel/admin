...
<div class="box-header with-border">
    <h2 class="box-title" data-bind="text: $root[$root.primaryKey]() ? '<?php echo trans('administrator::administrator.edit') ?>' :
    '<?php echo trans('administrator::administrator.createnew') ?>'"></h2>
</div>
<form class="edit_form" data-bind="visible: !loadingItem(), submit: saveItem" role="form">
    <div class="box-body">
    ...
    <div class="box-footer" style="background: #D9E4EC;">
        <div class="control_buttons">
            <!-- ko if: $root[$root.primaryKey]() -->
                <input type="button" class="btn btn-default" value="<?php echo trans('administrator::administrator.close') ?>"
                    data-bind="click: closeItem, attr: {disabled: $root.freezeForm() || $root.freezeActions()}" />

                <!-- ko if: actionPermissions.delete -->
                    <input type="button" class="btn btn-danger" value="<?php echo trans('administrator::administrator.delete') ?>"
                        data-bind="click: deleteItem, attr: {disabled: $root.freezeForm() || $root.freezeActions()}" />
                <!-- /ko -->

                <!-- ko if: actionPermissions.update -->
                    <input type="submit" class="btn btn-success" value="<?php echo trans('administrator::administrator.save') ?>"
                        data-bind="attr: {disabled: $root.freezeForm() || $root.freezeActions()}" />
                <!-- /ko -->
            <!-- /ko -->

            <!-- ko ifnot: $root[$root.primaryKey]() -->
                <input type="button" class="btn btn-default" value="<?php echo trans('administrator::administrator.cancel') ?>"
                    data-bind="click: closeItem, attr: {disabled: $root.freezeForm() || $root.freezeActions()}" />
                <!-- ko if: actionPermissions.create -->
                    <input type="submit" class="btn btn-success" value="<?php echo trans('administrator::administrator.create') ?>"
                        data-bind="attr: {disabled: $root.freezeForm() || $root.freezeActions()}" />
                <!-- /ko -->
            <!-- /ko -->
