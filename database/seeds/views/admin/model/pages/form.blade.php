                            ...
                            <div class="col-md-12">
                                ...
                                @foreach($arrayFields as $key => $arrCol)
                                    @if($arrCol['visible'] && $arrCol['editable'])
                                        <?php $tmpClass = in_array($key, ['content', 'websites']) ? 'col-md-12' : 'col-md-6'; ?>
                                        <div class="{{$tmpClass}}">
                                            <div class="form-group {{$errors->has($arrCol['field_name']) ? 'has-error' : null}}">
                                                <?php $tmpClass = in_array($key, ['content', 'websites']) ? 'col-md-2' : 'col-md-4'; ?>
                                                <label class="{{$tmpClass}} control-label" for="{{$arrCol['field_name']}}">
                                                    {!! $arrCol['title'] !!}
                                                </label>
                                                <?php $tmpClass = in_array($key, ['content', 'websites']) ? 'col-md-10' : 'col-md-8'; ?>
                                                <div class="{{$tmpClass}}">
